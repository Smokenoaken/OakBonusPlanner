...
local function CreateCogButton(parent, size)
    ...
    button:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Open the minimap-button visibility options.", 1, 1, 1, true)
        GameTooltip:Show()
    end)
    button:SetScript("OnLeave", function(self)
...
local function UpdateRow(row, data, top, index)
    ...
    row.rowData = data
    row.icon:SetTexture(data.artwork or "Interface\\Icons\\INV_Misc_QuestionMark")
    row.icon:SetDesaturated(not data.artwork)
    row:Show()
